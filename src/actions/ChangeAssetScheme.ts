...
export class ChangeAssetScheme extends Action<ChangeAssetSchemeTx> {
    ...
    protected apply(state: State) {
        ...
        if (changes.approver) {
            console.log(
                `change approver: ${(assetScheme.approver || "[null]").toString()} -> ${
        ...
        if (changes.registrar) {
            console.log(
                `change registrar: ${(assetScheme.registrar || "[null]").toString()} -> ${
        ...
        if (changes.allowedScriptHashes) {
            console.log(
                `change registrar: ${assetScheme.allowedScriptHashes.map(x => x.toString())}` +
